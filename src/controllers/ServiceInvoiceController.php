...
class ServiceInvoiceController extends Controller
{
    ...
    //OLD COMMENTED ON 8TH AUGUST 2023
    ...
    public function getCustomerAddress(Request $request)
    {
        // dd($request->all());
        try {
            if (isset($request->data['customer_edit']) && $request->data['customer_edit'] == 'yes') {
                if (isset($request->data['customer_id']) && $request->data['customer_id']) {
                    $customer = Customer::find($request->data['customer_id']);

                    if(!empty($customer->gst_number) && $customer->gst_number != 'Not available'){
                        $bdo_response = Customer::getGstDetail($customer->gst_number);
                        if (isset($bdo_response->original) && $bdo_response->original['success'] == false) {
                            return response()->json([
                                'success' => false,
                                'error' => 'BDO Error',
                    ...
                    $customer_address = Address::where('address_of_id', 24)
            ...
            }else if (isset($request->data['blocked']) && $request->data['blocked'] == 2) {
                return response()->json([
                    'success' => false,
                    'error' => 'The Customer Master Stopped in AX 12',
                ]);
            }else{
                $axUrl = "GetNewCustomerAddress_Search";
                if(Auth::user()->company_id == 1){
