...
class HondaServiceInvoiceApprovalController extends Controller {
	...
	public function getServiceInvoiceApprovalList(Request $request) {
		...
		$cn_dn_approval_list = HondaServiceInvoice::
		//withTrashed()
			select(
				...
				'honda_service_invoices.branch_id',
		...
		if (Entrust::can('Honda CN/DN Approval 1 View All')) {
			...
			$cn_dn_approval_list = $cn_dn_approval_list->whereIn('honda_service_invoices.created_by_id', $sub_employee_based);
		} else {
			$cn_dn_approval_list = [];
		}
		return Datatables::of($cn_dn_approval_list)
			...
			->addColumn('action', function ($cn_dn_approval_list) use ($approval_status_id) {
				...
				return '
				<a href="#!/service-invoice-pkg/honda-cn-dn/approval/approval-level/' . $approval_status_id . '/view/' . $type_id . '/' . $cn_dn_approval_list->id . '" class="">
				                    <img class="img-responsive" src="' . $img_view . '" alt="View" />
				                	</a>
				';
	...
	public function updateApprovalStatus(Request $request) {
		...
		DB::beginTransaction();
		try {
			$approval_status = HondaServiceInvoice::find($request->id);
			...
			$approved_status = new HondaServiceInvoiceController();
				...
				if ($approval_status->status_id == $approval_levels->name) {
					...
					if (isset($r['api_logs'])) {
						foreach ($r['api_logs'] as $api_log) {
							$api_data = HondaServiceInvoice::apiLogs($api_log);
						}
					}
					if (!$r['success']) {
						// DB::rollBack();
	...
	public function updateMultipleApproval(Request $request) {
		$send_for_approvals = HondaServiceInvoice::whereIn('id', $request->send_for_approval)->where('status_id', 2)->pluck('id')->toArray();
		...
		if (count($send_for_approvals) == 0) {
			...
			try {
				foreach ($send_for_approvals as $key => $value) {
					...
					$send_approval = HondaServiceInvoice::find($value);
	...
	public function updateIRNStatus($id) {
		...
		try {
			...
			$service_invoice = HondaServiceInvoice::find($id);
